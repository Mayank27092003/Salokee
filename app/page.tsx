...
export default function Home() {
  ...
  return (
    <main className="h-screen bg-neutral-50 relative flex flex-col overflow-hidden font-sans">
      {/* HEADER */}
      <header className="w-full z-50 flex justify-between items-center px-6 lg:px-12 py-5 bg-white/80 backdrop-blur-xl border-b border-neutral-200/50 shrink-0 sticky top-0">
        <h1
          onClick={() => router.push("/")}
        ...
        <div className="flex gap-4 items-center">
          <button
            ...
            className="px-5 py-2.5 rounded-full border border-neutral-200 text-neutral-700 text-sm font-semibold hover:bg-neutral-100 hover:border-neutral-300 transition-all shadow-sm bg-white"
          >
            We're hiring
          </button>
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 transition-opacity duration-500 delay-150 relative ${activeCard === "customers" ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                {countries.map((c) => (
                  <button
                    ...
                    className="bg-white/10 backdrop-blur-xl hover:bg-white/20 text-white border border-white/20 hover:border-white/40 shadow-xl rounded-2xl py-4 px-5 flex items-center gap-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl text-left overflow-hidden relative group/btn"
                  >
                    ...
                    <img src={`https://flagcdn.com/${c.countryCode}.svg`} alt={c.name} className="w-8 h-auto rounded-[4px] object-cover shadow-[0_2px_8px_rgba(0,0,0,0.3)] relative z-10" />
                    <div className="flex flex-col relative z-10">
                      <span className="text-xs text-white/70 font-bold uppercase tracking-wider">{c.code}</span>
